...
$g$ is the $q$-power Frobenius $\mathbb{F}_{q^n} \to \mathbb{F}_{q^n}$. 
...
to~\cite{GaSh92}, where it was credited to Kaltofen). As a first
application, given $g \in G$ (by means of $\gamma:=g(\bar x)$), we can
...
\end{lemma}
\begin{proof}
(Compare \cite[Lemma~4]{Kaltofen}) For $m=1,\dots,r$, suppose we have computed 
...
e_1 \cdots e_r$; without loss of generality, all $e_r$'s are at least
equal to one.

Moreover, let $s_i = \lceil \sqrt{e_i \rceil}$ for $ 1\leq i \leq r$.
