...
of $\K/\F$. The map $\varphi = \underset{d \vert n}{\oplus} \varphi_d$ where,
\begin{equation}
\begin{split}
\varphi_1: \F[G] \longrightarrow M_m(\F) \\
...
I_{m-1}& 0
\end{array} \right] \\
\varphi_d: \F[G] \longrightarrow M_{m}(\F(\zeta_d))\\
\sigma \longrightarrow \mathrm{Diag}(\zeta_d, \zeta_d^r, \ldots , \zeta_d^{r^{m-1}})\\
\tau \longrightarrow 
\left[ \begin{array}{l|l}
0 & 1\\
\hline
...
\end{split}
\end{equation}
where $\zeta_d$ is a primitive $d$-th root of unity, is an injective homomorphism.
\end{proposition}
