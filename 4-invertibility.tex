...
\begin{lemma}
  ...
  $\tilde{O}(\varphi(mm'))$, and one can apply $\gamma$ and its
  inverse to any inputs in time $\tilde{O}(\varphi(mm'))$.
\end{lemma}
\begin{proof}
  ...
  polynomial $\bar P$ of degree less than $mm'$, which we reduce modulo
  $\Phi_{mm'}$ (the latter is obtained by the composed product
  algorithm of~\cite{BoFlSaSc06} in quasi-linear time).

  By~\cite[Theorem~8.8.7]{BaSh96}, $s = O(\varphi(s) \log(\log(s)))$,
  so that $s$ is in $\tilde{O}(\varphi(s))$. Thus, we can reduce 
  $\bar P$ modulo $\Phi_{mm'}$ in $\tilde{O}(\varphi(mm'))$ operations. 
