...
Together with Proposition \ref{prop:abelian}, the above result proves the first part of
...
polynomial ring.  Here we will use an explicit injective
...
Although the second algorithm above gives a quadratic cost, the first algorithm 
gives a better cost in many cases. More precisely, if $s \leq m$ then the first algorithm
is subquadratic in the size of input. Moreover it is known that if $s$ is prime, then
the number $$\dfrac{m- \gcd(m,r-1)}{s}$$ is a positive integer which implies $s \leq m$ 
(see \cite[Theorem 47.12, Corollary 47.14 ]{Curtis}).

Corollary \ref{coro:test_meta} provides the last part of the proof of Theorem \ref{thm:main}.
