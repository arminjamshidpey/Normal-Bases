...
by \citeN{Giesbrecht}, with a cost of $O(n^2+n\log q)$ operations in
$\FF_q$.  A faster randomized algorithm is introduced by
\citeN{Kaltofen}, with a cost of $O(n^{1.815}\log q)$ operations in
...
0$), by leveraging their quasi-linear time algorithm for {\em modular
  composition}~\cite{KeUm11}. \citeN{LenstraNormal} introduced a
deterministic algorithm to construct a normal element which uses
$O(n^{O(1)})$ operations in $\FF_{q^n}/\FF_q$.  To the best of our
knowledge, the algorithm of \citeN{AugCam94} is the most efficient
...
In this paper we present a new randomized algorithm for finding a
...
This paper is written under the point of view of asymptotic complexity
