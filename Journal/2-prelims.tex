...
matrix $\mat M$ and using linear algebra to compute its
...
matrix $\mat M$ above is the matrix of left multiplication by the orbit
sum
$$S_\alpha:= \sum_{g \in G} g(\alpha)g \in \K[G],$$ where we index
...
\end{equation}
Note $\ell(\mat M_\K(S_\alpha))$ is equal to $\mat
M_\F(s_{\alpha,\ell})$, that is, the multiplication matrix by
$s_{\alpha,\ell}$ in $\F[G]$, where, as above, we index rows by
$g_1,\dots,g_n$ and columns by $g_1^{-1},\dots,g_n^{-1}$.  Thus, once
$s_{\alpha,\ell}$ is known, we are left with testing whether it is a
unit in $\F[G]$.  In the next two sections, we address the respective
questions of computing $s_{\alpha,\ell}$, and testing its
invertibility in $\F[G]$.
