...
division in $\F[G]$, specifically for abelian and metacyclic groups
$G$: given elements $\beta,\eta$ in $\F[G]$, for a field $\F$ and a
group $G$, determine whether $\beta$ is a unit in $\F[G]$, and if so,
compute $\beta^{-1}\eta$. As well as being necessary in our
application to normal bases, we believe these problems are of
...
testing and division for the families of group seen so far, abelian
and metacyclic. For the former, starting from a suitable presentation
of $G$, we give a softly linear-time algorithm to find an isomorphic
...
alsos make it possible to find its inverse in $\F[G]$ (or to
compute products in $\F[G]$) in softly-linear time (we are not aware
of previous results of this kind). 
...
\end{proposition}
The proof occupies the rest of this paragraph.  Note that together
with Proposition \ref{prop:abelian}, the claim on the cost of
invertibility testing proves the first part of Theorem \ref{thm:main};
the second part of this proposition will allow us to prove
Theorem~\ref{thm:main2} in the next section.
...
equivalent to $BB''$ being invertible, that is, to all its components
...
for a metacyclic group $G$. A possibility would be to rely on the
Wedderburn decomposition of $\F[G]$, but the structure of group
algebras of metacyclic groups is not straightforward to exploit; see
for instance \citep[\S 47]{Curtis} for algebraically closed $\F$,
or~\citep{Decomposition} for dihedral and quaternion groups over
$\F=\Q$. Instead, we will highlight the structure of the
...
Take $\beta$ in $\F[G]$. In Section~\ref{sec:pre}, we introduced the
matrix $\mat M_\F(\beta)$ of left multiplication by $\beta$ in
$\F[G]$, where columns and rows were indexed using an unspecified
ordering of the group elements. We will now be more specific, and
reorder the rows and columns of $\mat M_\F(\beta)$ using the two
presentations of $G$ seen in \eqref{pres1} and~\eqref{pres2},
in order to highlight its structure.
In what follows, for non-negative integers $a,b,c$, we will write
$\beta_{a,b,c}$ for the coefficient of $\tau^a \sigma^b \tau^c$ in 
the expansion of $\beta$ on the $\F$-basis of $\F[G]$. 
...
\end{proposition}
Combined with Proposition~\ref{prop:sum_meta}, the former statement
...
%% $F_i \bmod (z^m-1)=0$, for $0 \leq i < s$. Since all $F_i$'s have degree 
%% less than $m$, they are all zero.
%% \end{proof}
...
%% which implies $s \leq m$ (see \citep[Theorem 47.12, Corollary 47.14
%% ]{Curtis}).
...
%%   \citeyearpar{Wiedemann86} algorithm,
...
%%   of~\citet[Section~4]{KaSa91} to compute the rank of all these matrices
...
%%   algorithms of~\cite{AcCoMa03} and \cite{DaMMMScXi06}. Using $\tilde{O}(sm)$
