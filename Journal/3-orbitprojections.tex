...
The original algorithm of \cite{KalSho98} relies on the properties of the
...
of~\cite{KalSho98}, Lemmata 3, 4 and~8, now considering rectangular matrix
...
upper bounds on $\omega(1.3)$ and $\omega(1.4)$ given by~\cite{LeGall}, and
the fact that $k \mapsto \omega(k)$ is convex~\citep{LoRo83}. In particular,
...
\cite{KalSho98} also have such a discussion). Seen as an
...
{\em transposition principle}~\citep{KaKiBs88,CaKaYa89} implies that if
