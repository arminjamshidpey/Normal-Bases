...
If $G$ is cyclic, \cite{Giesbrecht} avoided the determinant
...
%%  This gives an idea to modify Algorithm
%% \ref{Alg:Naive}. Instead of writing $M_G(x)$ and test its
%% invertiblity, we can write $\osum{G}{\K}$ and test if it is invertible
%% in $\K[G]$. Although testing the invertibility of $\osum{G}{\K}$ might
%% be efficient in comparison to computing the determinant of a matrix in
%% $\K$, we prefer to do the computations over $\F$ rather than $\K$. The
%% following lemma comes handy to pass the computations from $\K$ to
%% $\F$.
