...
state a part of such a proof up to the point that we need. Assume $\K/\F$
is a finite Galois extension with Galois group
$G = \lbrace g_1 , \ldots , g_n \rbrace$. If $x \in \K$ is a normal
element, then clearly 
\begin{equation}
  \label{eq:fstrow}
  \sum_{j=1}^n 
...
\begin{equation} \label{eq:otherrow}
 ...
 c_j g_i g_j(x)=0.
\end{equation}
Using \eqref{eq:fstrow} and \eqref{eq:otherrow} one can form a linear system $M_G(\alpha)\textbf{v} = \textbf{0}$ where 
 $$ M_G(x) =
\begin{bmatrix}
...
\end{bmatrix}. 
 $$
 Equation \eqref{eq:otherrow} shows that $M_G(x)$ is non-singular. The rest of the proof shows that there exists $\alpha \in \K$ with $\det(M_G(\alpha))\neq 0$.
